...
export function addPartListData(data) {
  return async (dispatch) => {
    let response = await Axios.post("http://localhost:8002/part/add", data);
    dispatch(fetchPart());
  };
}
...
export function fetchPart(data) {
  return async (dispatch) => {
    let response = await Axios.get("http://localhost:8002/part/get", {
      params: {
        date: data,
        id: "",
      },
...
export function deletePartListData(data) {
  return async (dispatch) => {
    let response = await Axios.delete(
      `http://localhost:8002/part/delet/${data}`
    );
    dispatch(fetchPart());
  };
}
...
export function editePartListData(data, id) {
  return async (dispatch) => {
    let response = await Axios.patch(
      `http://localhost:8002/part/edit/${id}`,
      data
    );
    dispatch(fetchPart());
  };
...
export function getDataById(dataId, data) {
  return async (dispatch) => {
    ...
    let response1 = await Axios.get("http://localhost:8002/part/fetch");
